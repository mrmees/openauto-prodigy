...
bool TouchRouter::dispatch(int slot, float x, float y, TouchEvent event) {
    ...
    // Copy zone list under lock (zones are rarely updated, copies are small)
    std::vector<TouchZone> snapshot;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        snapshot = zones_;
    }
    ...
    if (!claimed.empty()) {
        ...
        // Claimed zone was removed (stale claim cleared by setZones, but
        // could be a race). Clear and fall through.
        claimedZone_[slot].clear();
    }
    ...
    // (snapshot is already sorted by priority descending)
    for (const auto& zone : snapshot) {
