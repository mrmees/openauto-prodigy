# Requirements Archive: v0.5.3 Widget Grid & Content Widgets
...
- [x] **NAV-01**: AA navigation turn-by-turn widget shows maneuver icon, road name, and distance to next turn
- [x] **NAV-02**: Maneuver icon renders phone-provided PNG (via QQuickImageProvider), falls back to Material icon
- [x] **NAV-03**: Widget shows "No active navigation" with muted icon when nav is inactive
- [x] **NAV-04**: Tapping the navigation widget activates AA fullscreen view
- [x] **NAV-05**: Navigation data exposed to QML via C++ NavigationDataBridge with Q_PROPERTYs

### Unified Media

- [x] **MEDIA-01**: Unified Now Playing widget shows track title, artist, and playback controls
- [x] **MEDIA-02**: Widget displays AA media metadata when AA is connected, BT A2DP metadata as fallback
- [x] **MEDIA-03**: Source indicator label shows "via Android Auto" or "via Bluetooth"
- [x] **MEDIA-04**: Playback controls (prev/play-pause/next) map to appropriate source (AVRCP for BT, InputEvent for AA)
- [x] **MEDIA-05**: AA media metadata exposed to QML via C++ MediaDataBridge (currently parsed but not surfaced)
- [x] **MEDIA-06**: Replaces current BT-only NowPlayingWidget
...
| NAV-02 | Phase 06 | Complete |
| NAV-03 | Phase 06 | Complete |
| NAV-04 | Phase 06 | Complete |
| NAV-05 | Phase 06 | Complete |
| MEDIA-01 | Phase 06 | Complete |
| MEDIA-02 | Phase 06 | Complete |
| MEDIA-03 | Phase 06 | Complete |
| MEDIA-04 | Phase 06 | Complete |
| MEDIA-05 | Phase 06 | Complete |
| MEDIA-06 | Phase 06 | Complete |
