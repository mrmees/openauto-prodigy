# Requirements Archive: v0.6.5 Widget Refinement

**Archived:** 2026-03-21
**Status:** SHIPPED

For current requirements, see `.planning/REQUIREMENTS.md`.

---

# Requirements — v0.6.5 Widget Refinement

**Defined:** 2026-03-20
**Core Value:** A person with a Raspberry Pi 4 and a touchscreen can install this, pair their phone, and get a reliable wireless Android Auto experience — every time, without SSH.

## Widget Refinement (WR)

- [x] **WR-01**: Every widget renders correctly at all valid span combinations on the DFRobot 7" (1024x600) display
- [x] **WR-02**: Every widget renders correctly at all valid span combinations on the Pi Official 7" (800x480) display
- [x] **WR-03**: Widget text and icons are legible at 1x1 minimum span on both target displays
- [x] **WR-04**: All user-requested visual refinements from the preview review session are applied

## Date Widget (DT)

- [x] **DT-01**: Standalone date widget shows day-of-week and date
- [x] **DT-02**: Date widget has responsive breakpoints (1x1 compact → larger spans show more detail)
- [x] **DT-03**: Date widget is available in the widget picker with appropriate metadata

## Clock Cleanup (CL)

- [x] **CL-01**: Clock widget (all 3 styles) no longer displays date or day-of-week at any span
- [x] **CL-02**: Clock widget layout is adjusted for time-only display (no wasted space where date was)

## Hardware Verification (HW)

- [x] **HW-01**: All widgets verified on Pi hardware after refinement pass

## Out of Scope

| Feature | Reason |
|---------|--------|
| New widget types beyond date | Can add via /gsd:insert-phase if needed |
| Widget preview tool improvements | Dev tooling, not product scope |
| Config schema changes | No new config fields needed for visual refinements |

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| WR-01 | Phase 23 | Complete |
| WR-02 | Phase 23 | Complete |
| WR-03 | Phase 23 | Complete |
| WR-04 | Phase 23 | Complete |
| DT-01 | Phase 22 | Complete |
| DT-02 | Phase 22 | Complete |
| DT-03 | Phase 22 | Complete |
| CL-01 | Phase 22 | Complete |
| CL-02 | Phase 22 | Complete |
| HW-01 | Phase 24 | Complete |

**Coverage:**
- v0.6.5 requirements: 10 total
- Mapped to phases: 10
- Unmapped: 0

---
*Requirements defined: 2026-03-20*
