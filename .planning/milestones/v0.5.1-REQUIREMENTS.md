# Requirements Archive: v0.5.1 DPI Sizing & UI Polish
...
- [x] **AAT-01**: Phone-sent UiConfigRequest (0x8011) is parsed into day/night token maps by VideoChannelHandler
- [x] **AAT-02**: UpdateHuUiConfigResponse (0x8012) ACCEPTED is sent back to the phone
- [x] **AAT-03**: Tokens flow through signal chain to ThemeService and are applied (if connected-device active) or cached (always)
- [x] **AAT-04**: All theme keys use hyphens matching the AA wire token format
...
- [x] **FIX-01**: AA video content is fully visible with no cropping when navbar is shown (margins negotiated correctly)
- [x] **FIX-02**: Touch coordinates align with visible AA content (tapping a button hits that button)
- [x] **FIX-03**: Existing SDP margin calculation unit tests continue to pass after rendering changes
...
| AAT-01 | Phase 3.1 | Complete |
| AAT-02 | Phase 3.1 | Complete |
| AAT-03 | Phase 3.1 | Complete |
| AAT-04 | Phase 3.1 | Complete |
| FIX-01 | Phase 3.4 | Complete |
| FIX-02 | Phase 3.4 | Complete |
| FIX-03 | Phase 3.4 | Complete |
...
**Coverage:**
- v0.5.1 requirements: 33 total (4 descoped: NAV-03..06 — protocol limitation)
- Mapped to phases: 33
- Unmapped: 0
