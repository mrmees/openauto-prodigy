...
3. Per-payload observables (companion app running, paired via API v1):
   ...
   - Time: journal shows the controlled clock-step entry (`journalctl -u openauto-prodigy.service | grep -i clock` — SYSTEM unit, not `--user`; requires induced drift ≥30 s, an NTP-synced Pi never steps).
...
>   - **CORRECTION (2026-07-13, post-bench investigation): the FAIL diagnosis was a false positive.** The wiring has existed since `8e7878c` (2026-07-06) — `main.cpp` connected `timeReported`/`timezoneReported` to static mirrors of `adjustClock` — and that commit is an ancestor of the deployed binary. The verdict came from a grep that missed `main.cpp` plus two observability traps: the journal check in step 3 above says `journalctl --user` but the unit is SYSTEM-level (always-empty result), and an NTP-synced bench Pi never accumulates the 30 s drift the step needs, so the clock-step line can never appear without induced drift. The row was never live-tested. Investigation DID find three real defects the re-validation must cover: (1) `timedatectl set-time` parses a bare timestamp as LOCAL time while both copies passed a UTC wall-clock string — clock would step wrong by the zone offset on any non-UTC Pi (fixed: explicit ` UTC` suffix); (2) no polkit rule existed for `set-timezone`, so the v1.1 timezone step would be denied (fixed: action added, rule rescoped from hardcoded user `matt` to `isInGroup("bluetooth")` matching the other rules); (3) the logic lived untested in two copies (fixed: extracted to `ClockSyncService`, unit-tested, `main.cpp` wired to it; legacy copy untouched, dies at B2). Re-validation procedure: disable NTP, induce ≥35 s drift, send TimeReport via trusted-localhost client, check `journalctl -u openauto-prodigy.service` for `ClockSync: clock adjusted`, verify wall clock, restore NTP; same session validates a `set-timezone` round-trip.
>   - **RE-VALIDATED (2026-07-13, post-fix deploy): time row PASSES.** ClockSyncService binary deployed + new polkit rule installed; NTP disabled, Pi clock stepped back 2 min, TimeReport sent over the real API v1 wire path (scratch localhost client, TCP 9810, hand-encoded protobuf). Journal: `ClockSync: clock adjusted by 119922 ms`; Pi wall clock agreed with true time within ~0.6 s afterward **on an America/Chicago Pi** (proves the UTC-suffix fix — pre-fix code would have landed 5 h off). Timezone round-trip: report with `timezone_id=America/Denver` → `ClockSync: timezone adjusted to "America/Denver"` (this call was polkit-DENIED before the rule fix) with NO second clock step (30 s threshold live-validated), then a Chicago report stepped it back. NTP re-enabled, `System clock synchronized: yes`, zone restored. **B2 blocker cleared.**
>   - **RE-VALIDATED AGAIN (2026-07-13, gate re-run finding): NTP-enabled stepping now works — the pre-fix path could NEVER step in the car.** Codex P1: timedated refuses `SetTime` while NTP is enabled — even offline/unsynced, the normal state of an RTC-less car unit; the first validation masked this by manually disabling NTP. Refusal reproduced verbatim on the Pi (`Failed to set time: Automatic time synchronization is enabled`). Fix: every step runs inside a `set-ntp false` → `set-time` → `set-ntp true` sandwich (NTP restored on failure too; polkit rule gained `set-ntp`). Live proof with NTP ENABLED + NTP traffic blocked (udp/123 dropped, simulating no internet): induced −2 min drift, TimeReport over the wire → `ClockSync: clock adjusted by 119865 ms`; timesyncd's own journal shows the sandwich (`Stopped` in drifted time, `Started` in corrected time — the 2-min timestamp jump IS the step). Bench note: on an internet-connected Pi, timesyncd wakes on clock changes and re-corrects within ~1 s, so drift induction for this test REQUIRES blocking udp/123 (or pulling uplink); also an active AA session floods the journal — grep a window, not `-n` tails. iptables rule removed after; NTP re-sync confirmed.
