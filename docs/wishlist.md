...
- **Cross-build speed (20 min → ~5)** — Matthew flagged 2026-07-07: `cross-build.sh` builds ALL targets including ~30 ARM test binaries that can never run here (each links the ~250MB static lib), and `build-pi/` sits on the drvfs /mnt/e mount (the same slow-9p problem that moved the native build to ext4). Fix: deploy builds use `cmake --build . --target openauto-prodigy` (or -DBUILD_TESTING=OFF), relocate build-pi to ext4 (docker volume or ~/builds/openauto-prodigy-pi bind mount), add ccache inside the container. Native WSL build workflow unchanged.
