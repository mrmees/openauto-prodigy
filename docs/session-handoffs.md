...
**Open decision (Matthew):** proxy-route teardown on companion disconnect — recommendation drafted (route follows the reporting session: owner-tracked clear in `sessionClosed`; daemon-side health auto-teardown filed as follow-up for the vanished-phone case). NOT implemented pending the ask.

**Status:** Complete and verified on WSL. **Pi deploy NOT run** (standard next step: cross-build rsync + restart; then journal-check that the server_id mints exactly once and API clients see `api_version_minor=1`; a corrupt-config recovery check is cheap: garbage the config, boot, confirm `.corrupt` + defaults).

**Verification:** per-task make + full ctest at every step (104/104 throughout — no new test binaries, 15 new test functions inside existing suites); `sg docker -c ./cross-build.sh` clean at dd2c68f and 079243f.
